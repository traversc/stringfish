...
Yes you are reading the graph correctly: some functions in `stringfish` are more than an order of magnitude faster than base R operations with a bit of multithreading (and sometimes without any multithreading). On large text datasets, this can turn minutes of computation into seconds. 
...
`stringfish` functions are not intended to exactly replicate their base R analogues. One difference is that `subject` parameters are always the first argument, to be easier to use with pipes (`%>%`). E.g., `gsub(pattern, replacement, subject)` becomes `sf_gsub(subject ,pattern, replacement)`.
